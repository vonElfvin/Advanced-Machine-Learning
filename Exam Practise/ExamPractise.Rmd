...
lines(XStar, pred2, col="blue")
legend("bottomright", legend=c("l=0.2", "l=1"), col=c("red", "blue"), lty=c(1,1))
get_covariance = function(x,xStar, sigmaN, K) {
  n = length(x)
  I = diag(n)
  V = K(xStar, xStar) - K(xStar, x) %*% solve(K(x,x) + sigmaN^2 * I) %*% K(x,xStar)
  return(V)
}
lm = lm(y~x)
sigmaN = sd(lm$residuals)
V1 = get_covariance(x, xStar, sigmaN, SEK1)
```
...
```{r}
set.seed(123)
bn.structure = hc(train)
bn.fit = bn.fit(bn.structure, train)

set.seed(123)
preds = c()
for(i in 1:1000){
  dist = cpdist(bn.fit, "S", (
    A == test[i,]$A & 
    T == test[i,]$T & 
    L == test[i,]$L & 
    B == test[i,]$B & 
    ...
    X == test[i,]$X &
    D == test[i,]$D))
  preds[i] = prop.table(table(dist))[2] > 0.5
}
table(test$S, preds)
```
